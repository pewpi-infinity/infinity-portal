...
# Rogers AI server with clean white theme and no API keys required.
#
# Run:
#   pip install flask requests beautifulsoup4 lxml
#   python3 rogers_white_nokeys.py
...
<body class="min-h-screen flex flex-col text-neutral-900">
  <!-- Header -->
  <div class="sticky top-0 z-30 glass border-b border-neutral-200 shadow-sm">
    <div class="mx-auto max-w-3xl px-4 py-4 flex items-center justify-between gap-3">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <div class="text-sm text-neutral-500">Powered by Infinity</div>
        </div>
      </div>
      <button class="rounded-lg border border-neutral-300 bg-white px-4 py-2 text-sm font-semibold shadow-sm hover:bg-neutral-50">Sign in</button>
    </div>
  </div>

  <!-- Main -->
  <div class="mx-auto max-w-3xl w-full flex-1 px-4 pt-6 pb-4">
    <div id="chat" class="h-[65vh] overflow-y-auto rounded-xl border border-neutral-200 bg-white shadow-inner p-4 flex flex-col gap-3"></div>

    <!-- Composer -->
    <div class="mt-4 flex items-end gap-2">
      <div class="flex-1 rounded-xl border border-neutral-300 bg-white px-4 py-3 shadow-sm">
      ...
      <button id="sendBtn" class="rounded-xl bg-gradient-to-r from-blue-600 to-purple-600 text-white font-semibold px-6 py-3 shadow-md hover:shadow-lg active:scale-95 transition-all">Send</button>
    </div>
    ...
    <!-- Info -->
    <div class="mt-6 rounded-xl border border-neutral-200 bg-white p-5 shadow-sm">
      <div class="font-bold text-lg mb-2">🤖 No API Keys Required</div>
      <ul class="list-disc pl-5 space-y-1 text-sm leading-relaxed">
        <li>Rogers aggregates answers from DuckDuckGo, Wikipedia, and Wikidata</li>
        <li>All queries run locally - no external API keys needed</li>
        <li>Two modes: <strong>Poll</strong> (parallel) or <strong>Chain</strong> (sequential)</li>
        <li>Clean, fast, and privacy-focused</li>
      </ul>
    </div>
  </div>
...
<script>
  ...
  // Welcome message
  addBubble("assistant", "Hello! I'm Rogers, your AI assistant. Ask me anything and I'll search multiple sources for you.");

  function addBubble(role, text) {
    const wrap = document.createElement("div");
    wrap.className = "flex " + (role === "user" ? "justify-end" : "justify-start");
    const b = document.createElement("div");
    b.className = "max-w-[85%] rounded-2xl px-4 py-3 shadow-sm " + 
      (role === "user" 
